    ...
    return struct {
        ...
        pub const Slice = struct {
            ...
            pub fn toMultiArrayList(self: Slice) Self {
                ...
                const aligned_ptr: *S = @ptrCast(@alignCast(unaligned_ptr));
                const casted_ptr = @as([*]align(@alignOf(S)) u8, @ptrCast(aligned_ptr));
                return .{
                    .bytes = casted_ptr,
                    .len = self.len,
        ...
        pub fn setCapacity(self: *Self, gpa: Allocator, new_capacity: u32) !void {
            ...
            inline for (fields, 0..) |field_info, i| {
                if (@sizeOf(field_info.type) != 0) {
                    const field = @as(Field, @enumFromInt(i));
                    mem.copyForwards(field_info.type, other_slice.items(field), self_slice.items(field));
                }
            }
            gpa.free(self.allocatedBytes());
            self.* = other;
